...
# SECRET_KEY = "secret_key"         # Секретный ключ для подписи JWT
# ALGORITHM = "HS256"               # Алгоритм для подписи JWT
# ACCESS_TOKEN_EXPIRE_MINUTES = 30  # Время действия токена


#ready
@authorization.post("/authorization/register")
async def register(user: User):
    ...
    user_db = await db.read_database('data_users', user.username,['user_name', 'password'])
    
    if user_db != None:
        raise HTTPException(status_code=400, detail="Username already registered") # name busy 
    # hash password 
    hashed_password = get_password_hash(user.password)
    success = await db.add_user_database_in_table('data_users', user.username, hashed_password)
...
@authorization.post("/authorization/login")
